...
.eqv RIGHT_LED            0xFFFF0010  # chua byte dieu khien den led ben phai
.eqv LEFT_LED         	  0xFFFF0011  # chua byte dieu khien den led ben trai
.text 
main: 
...
Loop: beq   $s4,2,input_number_2nd
      nop
      beq   $s4,2,input_number_2nd
      nop
      beq   $s4,2,input_number_2nd
      nop
      beq   $s4,2,input_number_2nd
...
showketqua: 
      li    $t9,0
      li    $t8,0
      slt   $k1,$s7,$0              # kiem tra ket  qua la so am hay khong $s7 < 0 => $k1 =1
      bne   $k1,$0,loisoam
      div   $t8,$s7,10
      mfhi  $t9
...
napgiatricholed: 
      li     $t2,RIGHT_LED   # hien thi den LED trai
      add    $s0,$zero,$t9 # truyen bien left
      jal    hienthi
      nop
      li     $t2,LEFT_LED  # hien thi den LED phai
...
hienthi:                      #chuong trinh con
      showleds_0: 
            bne   $s0,0,showleds_1  # case $s0 = 0
            li    $t4,ZERO
            j     napgiatri
      showleds_1: 
            bne   $s0,1,showleds_2  # case $s0 = 1
            li    $t4,ONE
            j     napgiatri
      showleds_2: 
            bne   $s0,2,showleds_3  # case $s0 = 2
            li    $t4,TWO
            j     napgiatri
      showleds_3: 
            bne   $s0,3,showleds_4  # case $s0 = 3
            li    $t4,THREE
            j     napgiatri
      showleds_4: 
            bne   $s0,4,showleds_5  # case $s0 = 4
            li    $t4,FOUR
            j     napgiatri
      showleds_5: 
            bne   $s0,5,showleds_6  # case $s0 = 5
            li    $t4,FIVE
            j     napgiatri
      showleds_6: 
            bne   $s0,6,showleds_7  # case $s0 = 6
            li    $t4,SIX
            j     napgiatri
      showleds_7: 
            bne   $s0,7,showleds_8  # case $s0 = 7
            li    $t4,SEVEN
            j     napgiatri
      showleds_8: 
            bne   $s0,8,showleds_9  # case $s0 = 8
            li    $t4,EIGHT
            j     napgiatri
      showleds_9: 
            bne   $s0,9,showleds_df # case $s0 = 9
            li    $t4,NINE
            j     napgiatri
      showleds_df:  jr $ra
      napgiatri:    
...
getInt1: 
  ...
  #-------------------------------------------------------------------------
    ...
    #------------------------------------------------------------------------
  # Thu tuc quet cac phim o hang 2 va xu lyinput_number_2nd
  # Tham so truyen vao: 
  # Tra ve:
  #------------------------------------------------------------------------
getInt2: 
      ...
      case_7:   li $t5,0xffffff82
            ...
            add $s1,$s1,$t6 # factor=factor*10+left
            j show2
      show2:    li $t2,RIGHT_LED   # hien thi den LED trai
            add $s0,$zero,$t6 # truyen bien left
            jal displayLED
            nop
            li $t2,LEFT_LED  # hien thi den LED phai
